...
sequenceDiagram
    participant User
    participant Pipeline as src.run_pipeline
    participant Downloader as src.download_corpora
    participant Spider as Scrapy archive_email
    participant Mongo as MongoDB raw_emails
    participant Exporter as src.export_dataset
    participant EDA as src.eda
...
`src.export_dataset` tạo 3 CSV chính:
...
Module chính: `src/preprocess_balance.py`.
...
| Path | Vai trò |
|---|---|
| `src/run_pipeline.py` | Orchestrate full 7-step pipeline |
| `src/download_corpora.py` | Download/extract packaged corpora and save to MongoDB |
...
| `src/export_dataset.py` | Export raw, clean full, balanced train CSV |
| `src/preprocess_balance.py` | Strong text cleaning and balancing logic |
| `src/eda.py` | EDA before/after processing |
| `src/train.py` | Train/evaluate Naive Bayes model |
| `src/predict.py` | Predict one input string using saved model |
| `src/validate_crawl.py` | Validate crawl summary has data |
| `src/check_data.py` | Check MongoDB and CSV output locations |
| `src/error_logging.py` | Configure `PIPELINE.log` and `ERRORS.log` |
